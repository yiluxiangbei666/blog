    ...
    <script>
        $(function () {
            ...
            $.ajax({
                ...
                dataType:"json", //json格式解析返回数据
                type:"post",
                url:"/usermoney/queryrewardrecord",
                error:function (XMLHttpRequest, textStatus, errorThrown) {
                    console.log(XMLHttpRequest.status); //状态码
                    console.log(XMLHttpRequest.readyState); //状态
                    console.log(textStatus); //错误信息
            ...
            $.ajax({
                ...
                success:function (data) {
                    var booklist= $("#addbooklistbody").empty();
                    var d=new Date();
                    let year = d.getFullYear();//获取当前年
                    var mon = d.getMonth() + 1; //获取当前月
                    var date = d.getDate(); //获取当前日
                    ...
                    var dateindex=year+"."+mon+"."+date+"."+hour+"."+minutes;
                    for(var i=0;i<data.length;i++){
                        var addbookrecord=data[i].toString();
                        console.log(addbookrecord);
                        var arr=new Array();
                        arr=addbookrecord.split(':');
                        let booklist= $("#addbooklistbody");
                        let tr= $("<tr></tr>");
                        let td1= $("<td></td>");
                        let td2= $("<td style=\"width: 100px;height:30px;\"></td>");
                        let td3= $("<td></td>");
                        let td4= $("<td></td>");
                        td1.text(dateindex);
                        td2.text(arr[0].toString().substring(0,15)+"...");
                        td3.text(arr[1]);
                        td4.text("已购买");
                        tr.append(td1);
                        tr.append(td2);
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    for(var i=0;i<data.length;i++){
                        ...
                        var arr=new Array();
                        arr=addbookrecord.split(':');
                        let booklist= $("#mybookcar");
                        let tr= $("<tr></tr>");
                        let td1= $("<td></td>");
                        let td2= $("<td style=\"width: 100px;height:30px;\"></td>");
                        let td3= $("<td></td>");
                        let td4= $("<td></td>");
                        let td5= $("<td onclick=\"deletemybookcar(this)\"></td>");
                        let td6= $("<td style=\"display:none\"></td>");
                        td1.text(dateindex);
                        td2.text(arr[0].toString().substring(0,15)+"...");
                        td3.text(arr[1]);
                        td4.text(arr[2]);
        ...
        function deletemybookcar(val) {
            var userid=${user.userid};
            var parent = $(val).parent();
            var title=parent.children().eq(5).text();
            var price=parent.children().eq(2).text();
            console.log(title+" "+price);
            $.ajax({
                data: {userid:userid,title:title,price:price},//向后台controller发送的数据
                dataType:"json", //json格式解析返回数据
                type:"get",
                url:"/deletemybookcar",
                error:function (XMLHttpRequest, textStatus, errorThrown) {
...
3、充值成功后，余额可能存在延迟现象，一般1到5分钟内到账，如有问题，请咨询客服；">
        ...
        <div class="row">
            <div class="col-xs-3" id="myScrollspy">
                <ul id="mylist" class="nav nav-tabs nav-stacked" data-spy="affix" data-offset-top="125">
                    <li><button type="button"  class="btn btn-primary"
                                data-toggle="button" style="width: 150px;height: 50px">我的书籍</button></li>
                    <li><button type="button" class="btn btn-primary"
                                data-toggle="button" style="width: 150px;height: 50px">收益提现</button></li>
                    <li><button type="button" class="btn btn-primary"
            ...
            <div class="content_right">
                ...
                <div class="two" style="display: none">二</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button data-dismiss="modal" id="trueaddmoney" type="button" class="btn btn-primary">
                        充值
                    </button>
                </div>
            </div><!-- /.modal-content -->
