...
<head>
    ...
    <script>
            ...
            if ("WebSocket" in window)
            {
                ...
                ws.onmessage = function (event)
                {
                    ...
                    else
                    {
                        ...
                        var data=msg.split(":");
                        console.log(data[2]);
                        var wechatcontent = $("#mychatarea");
                        var span=$("<span style=\'width: 90%\'></span>");
                        span.text("他发来消息----> "+data[2]);
                        var br=$("<br/>");
                        wechatcontent.append(span);
                        wechatcontent.append(br);
                    }
                };
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    for(var i=0;i<data.length;i++){
                        ...
                        var tr=$("<tr></tr>");
                        var td1=$("<td data-toggle=\"modal\" data-target=\"#chat\" onclick=\"mycart(this)\"></td>");
                        var td2=$("<td></td>");
                        var td3=$("<td></td>");
                        td1.text(arr[0]);
                        td2.text(arr[1]);
        ...
        var friendid;
        function mycart(val){
            friendid=$(val).parent().children().eq(0).text();
            var friendname=$(val).parent().children().eq(1).text();
            $("#myfriendId").text("我与"+friendname+"的聊天");
            $("#mychatarea").empty();
            var userid=${user.userid};
            $.ajax({
                //contentType:"application/json;charset=UTF-8",//发送数据以key=value的形式传递
                data:{toUser:userid},//向后台controller发送的数据
                dataType:"json", //json格式解析返回数据
                type:"get",
                url:"/getusermsglist",
                error:function (XMLHttpRequest, textStatus, errorThrown) {
                ...
                success:function (data) {
                    ...
                    for(var i=0;i<data.length;i++){
                            ...
                            var date=msg.split(":");
                            console.log(date[1]+" "+date[6]);
                            if(friendid==date[1]){
                                var wechatcontent = $("#mychatarea");
                                var span=$("<span style=\'width: 90%\'></span>");
                                span.text("他发来消息----> "+date[7]);
                                var br=$("<br/>");
                                wechatcontent.append(span);
                                wechatcontent.append(br);
                            }
        ...
        function send()
        {
            ...
            var wechatcontent = $("#mychatarea");
            var span=$("<span style=\'float: right;width: 50%\'></span>");
            span.text(message+"<----我");
            var br=$("<br/>");
            wechatcontent.append(span);
            wechatcontent.append(br);
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="mychattitle">
                        <span id="myfriendId"></span>
                    </h4>
                </div>
                <div class="modal-body">
                    <div id="mychatarea" style="width: 95%;height: 160px;overflow:auto;border: 1px solid red;margin: auto">

                    </div>
                    <div>
                        <textarea id="Msg" placeholder="输入您想发送的消息" style="width: 95%;margin-left: 15px;height: 40px"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default"
                    ...
                    <button onclick="send()" type="button" class="btn btn-primary">
                        发送
                    </button>
                </div>
            </div><!-- /.modal-content -->
